...
> `Read` 会先清洗文本：中文、代码块、URL、表格碎片等不会送进英文 TTS；复杂公式会调用本地 LLM 变成英文口语描述。
...
| 🧠 LLM 朗读稿 | Read 前先用本地 `translategemma:4b` 准备英文朗读稿：英文保留，中文翻英，公式转英文口语描述 |
| ∑ 公式友好 | MathJax/MathML/LaTeX 会优先提取语义公式；朗读时转成流畅口语，翻译时保留为可复制的 LaTeX 代码 |
| 🧭 上下文翻译 | 翻译请求可附带附近正文作为本地上下文，只用于术语和指代消歧，不会把上下文段落一起输出 |
...
4. 点击 `Read` 本地朗读 LLM 准备后的英文内容，或点击 `Translate` 使用本机 Ollama 模型翻译 🎉
