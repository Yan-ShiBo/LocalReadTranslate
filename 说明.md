...
| 🧹 默认清洗 | Read 前自动移除中文、代码、URL、表格残片和噪声符号 |
| ∑ 公式口语化 | 简单公式规则转换，复杂公式交给本地 `translategemma:4b` 生成英文口语描述 |
...
4. 点击 `Read` 本地朗读清洗后的英文内容，或点击 `Translate` 使用本机 Ollama 模型翻译 🎉

> ⌨️ **快捷键**：选中文本后按 **Ctrl+Shift+S** 也可直接朗读。
> 复杂公式默认使用服务端的 `OLLAMA_FORMULA_MODEL`，默认值为 `translategemma:4b`。
