...
> 如果在 Gemini 等页面右下角齿轮都没有出现，优先检查 Tampermonkey 是否允许在 `gemini.google.com` 运行，以及 Chrome 扩展的“站点访问权限”是否允许该域名；如果齿轮出现但选区按钮不出现，脚本会通过 `selectionchange` 兜底监听动态页面选区。
...
> Translation 设置栏里的 **Keep loaded** 会用 Ollama `keep_alive: -1m` 预加载并常驻当前模型；不用时点击 **Unload** 释放显存。
