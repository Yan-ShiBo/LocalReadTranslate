...
| 🧭 避让选区 | Read/Translate 按钮和译文卡片会根据选区自动选择上方或下方，减少遮挡正文 |
...
`start.bat` 和 `Kokoro TTS.pyw` 会直接定位 `kokoro-tts` 环境里的 Python，日常启动不需要先执行 `conda init`。
...
3. 如需开发版，可打开 GitHub raw 脚本：`https://raw.githubusercontent.com/Yan-ShiBo/local-tts-env/main/tts-userscript.js`
