# 🎙️ Kokoro TTS 本地划词朗读系统

> 在 Chrome 浏览器中选中任意英文文本，一键聆听最自然、最标准的 AI 语音朗读。
> 基于 **Kokoro TTS**（82M 参数，开源 SOTA 轻量模型），完全本地运行，无需联网、免费无限量使用。
...
│                          │                        │   127.0.0.1:5000        │
│   ① 用户选中英文文本      │  ◄───────────────────  │                         │
│   ② 弹出「🔊 朗读」按钮   │    audio/wav 音频流     │   ③ GPU 推理生成语音     │
...
3. 删除编辑器中的所有内容，将 `tts-userscript.js` 的全部内容粘贴进去
4. 按 **Ctrl+S** 保存
...
2. 打开任意英文网页
3. **选中一段英文文本** → 自动弹出 🔊 朗读按钮
4. 点击按钮 → 几乎瞬间 → 聆听标准地道的英文朗读 🎉
...
| `tts-userscript.js` | Tampermonkey 油猴脚本，划词选中后朗读（含设置面板） |
...
### Q：和 Ollama 有什么关系？
A：**没有关系**。Ollama 是大语言模型（LLM）运行工具，用于文字对话，不支持语音合成。Kokoro TTS 是专门的文本转语音模型，两者是不同的工具。
