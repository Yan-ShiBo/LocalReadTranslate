...
if not defined PYTHONW_EXE (
    for /f "delims=" %%P in ('conda run -n %CONDA_ENV% python -E -c "from pathlib import Path; import sys; p=Path(sys.executable); w=p.with_name('pythonw.exe'); print(w if w.exists() else p)" 2^>nul') do (
...
start "Local Read & Translate" "%PYTHONW_EXE%" -E "%~dp0windows_launcher.py"
exit /b 0
