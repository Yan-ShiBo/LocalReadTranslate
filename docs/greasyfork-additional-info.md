...
- 第一次真正没有历史状态时才显示 **Checking translation sources...**；之后重新打开设置或刷新网页时会先恢复上次的来源、模型和可用状态，再在后台静默检查并自动更新
- 测试翻译、切换来源/模型以及常驻/卸载后的后台刷新也会保留当前可见状态；切换来源、模型或目标语言会清除不再匹配的旧测试结果
...
当前用户脚本版本为 `1.15.6`（FastAPI `1.7.20`）。它不是单独安装就能工作的云端脚本：浏览器端始终需要本项目的本地 FastAPI 中介服务。
...
`1.15.6` 会在后台响应到达前短暂显示上次已知状态，但错误、超时或离线结果返回后会
立即替换旧状态，不会一直把缓存显示为当前事实。
