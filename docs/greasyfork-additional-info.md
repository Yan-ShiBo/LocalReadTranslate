...
- 上下文长度会按模型大小自动裁剪：4B 模型更短更稳，9B/14B/更大模型会逐级保留更多上下文
...
翻译、朗读稿准备和复杂公式口语化默认都使用 `translategemma:4b`。可在服务端通过 `OLLAMA_TRANSLATE_MODEL`、`OLLAMA_READ_MODEL`、`OLLAMA_FORMULA_MODEL` 覆盖，也可在脚本设置里切换当前翻译/朗读准备模型。如果第一次变慢，通常是 Ollama 正在加载模型。4B 模型会自动使用较短上下文，公式朗读也会优先采用保守字面规则；14B 模型会保留更多上下文。
